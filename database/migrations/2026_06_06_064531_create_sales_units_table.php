...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('sales_unit_translations', function (Blueprint $table) {
            $table->id();
            $table->foreignId('sales_unit_id')->constrained('sales_units')->cascadeOnDelete();
            $table->string('locale');
            $table->string('name')->nullable();
            $table->unique(['sales_unit_id', 'locale']);
            $table->index(['sales_unit_id', 'locale']);
        });
    }
